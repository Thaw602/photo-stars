...
// Video galaxy — separate smaller spiral galaxy placed further away
const VIDEO_GALAXY_SCALE = 0.30;
const VIDEO_GALAXY_Z_OFFSET = -0.8;
const VIDEO_GALAXY_Y_OFFSET = 0.25;
const VIDEO_CORE_SCALE = 0.35;
const VIDEO_CONNECT_DIST_FACTOR = 0.55;
...
function generatePhotoNodes(files: MediaFile[]): PhotoNode[] {
  ...
  // Video nodes: separate smaller spiral galaxy placed behind the main one
  for (let vi = 0; vi < videoIndices.length; vi++) {
    ...
    const baseSize = (1.5 + (1 - pos.r) * 2.8 + rng2() * 1.0) * 1.2;
  ...
  // Photo nodes: spiral arm distribution in main galaxy
...
export default function GalaxyCanvas() {
  ...
  const photoNodesRef = useRef<PhotoNode[]>([]); const planetsRef = useRef<Planet[]>([]);
  ...
  const frameCount = useRef(-1); const particlesRevealed = useRef(false); const revealStartTime = useRef(0); const revealProgress = useRef(0);
  ...
  const hitTest = useCallback((mx: number, my: number): number => {
    ...
    const w = canvas.clientWidth, h = canvas.clientHeight, cx = w / 2, cy = h / 2, s = Math.min(w, h) * 0.55;
    ...
    for (let i = 0; i < nodes.length; i++) { const n = nodes[i]; const result = project3D(n.x, n.y, n.z, cx, cy, s, cosX, sinX, cosY, sinY, zoomRef.current); if (!result) continue; const dx = mx - result.px, dy = my - result.py, dist = Math.sqrt(dx * dx + dy * dy); const projectedSize = n.baseSize / Math.max(result.depth + 2.2 / zoomRef.current, 0.3) * 0.6; const hitR = Math.max(8, projectedSize * 5); if (dist < hitR && dist < bestDist) { bestDist = dist; best = i; } }
    return best;
  }, []);
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => { const wasDragging = isDragging.current; isPointerDown.current = false; isDragging.current = false; if (!wasDragging) { const rect = canvasRef.current?.getBoundingClientRect(); if (!rect) return; const mx = e.clientX - rect.left, my = e.clientY - rect.top; if (!particlesRevealed.current) { const cx = rect.width / 2, cy = rect.height / 2, coreClickR = Math.min(rect.width, rect.height) * 0.10; const dist = Math.sqrt((mx - cx) ** 2 + (my - cy) ** 2); if (dist < coreClickR) { particlesRevealed.current = true; revealStartTime.current = performance.now(); return; } return; } const hit = hitTest(mx, my); if (hit >= 0) { const node = photoNodesRef.current[hit]; node.clickFlash = 1.0; const file = getFileByIndex(node.fileIndex); if (file) selectFile(file); } else if (hit <= -2) { const fileIndex = -(hit + 2); const foNode = photoNodesRef.current.find(n => n.fileIndex === fileIndex); if (foNode) foNode.clickFlash = 1.0; const file = getFileByIndex(fileIndex); if (file) selectFile(file); } } }, [hitTest, manifest, selectFile, uploadedPhotos]);
  ...
  useEffect(() => { if (!manifest) return; const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d'); if (!ctx) return; startTimeRef.current = performance.now(); const render = (timestamp: number) => { const elapsed = (timestamp - startTimeRef.current) / 1000; const dpr = Math.min(window.devicePixelRatio || 1, 2); const w = canvas.clientWidth, h = canvas.clientHeight, cx = w / 2, cy = h / 2, projScale = Math.min(w, h) * 0.55; if (particlesRevealed.current && revealProgress.current < 1) { const sinceReveal = (timestamp - revealStartTime.current) / 1000; revealProgress.current = Math.min(1, sinceReveal / REVEAL_DURATION); const t = revealProgress.current; revealProgress.current = 1 - Math.pow(1 - t, 3); } const rp = revealProgress.current; const zoom = zoomRef.current, zoomTarget = zoomTargetRef.current; if (Math.abs(zoom - zoomTarget) > 0.001) zoomRef.current += (zoomTarget - zoom) * 0.12; else zoomRef.current = zoomTarget; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.fillStyle = '#000000'; ctx.fillRect(0, 0, w, h); if (!isPointerDown.current) { const av = angularVelocity.current; const speed = Math.sqrt(av.x * av.x + av.y * av.y); if (speed > 0.00005) { rotationX.current = Math.max(-1.3, Math.min(1.3, rotationX.current + av.x)); rotationY.current += av.y; av.x *= INERTIA_DECAY; av.y *= INERTIA_DECAY; } else { angularVelocity.current = { x: 0, y: 0 }; rotationY.current += 0.0003; } } galaxyRotation.current += ARM_ROTATION_SPEED; for (let i = 0; i < planetsRef.current.length; i++) { const planet = planetsRef.current[i]; planet.orbitPhase += planet.orbitSpeed; const cosPhi = Math.cos(planet.orbitPhase), sinPhi = Math.sin(planet.orbitPhase); const baseX = cosPhi * planet.orbitRadius * DISK_RADIUS, baseZ = sinPhi * planet.orbitRadius * DISK_RADIUS; const cosInc = Math.cos(planet.orbitInclination), sinInc = Math.sin(planet.orbitInclination); const pertX = Math.sin(elapsed * 0.7 + i * 1.3) * 0.025, pertY = Math.cos(elapsed * 0.9 + i * 2.1) * 0.025, pertZ = Math.sin(elapsed * 0.5 + i * 0.7) * 0.025; planet.x = baseX + pertX; planet.y = baseZ * sinInc + pertY; planet.z = baseZ * cosInc + pertZ; planet.trail[planet.trailHead] = { x: planet.x, y: planet.y, z: planet.z }; planet.trailHead = (planet.trailHead + 1) % PLANET_TRAIL_LENGTH; } const meteor = meteorRef.current; if (!meteor.active && timestamp > nextMeteorTime.current) { const corner = Math.floor(Math.random() * 4); let sx: number, sy: number, ex: number, ey: number; const mw = w, mh = h; switch (corner) { case 0: sx = mw * 0.1; sy = mh * 0.1; ex = mw * 0.9; ey = mh * 0.9; break; case 1: sx = mw * 0.9; sy = mh * 0.1; ex = mw * 0.1; ey = mh * 0.9; break; case 2: sx = mw * 0.2; sy = mh * 0.9; ex = mw * 0.8; ey = mh * 0.1; break; default: sx = mw * 0.8; sy = mh * 0.9; ex = mw * 0.2; ey = mh * 0.1; break; } sx += (Math.random() - 0.5) * mw * 0.15; sy += (Math.random() - 0.5) * mh * 0.15; ex += (Math.random() - 0.5) * mw * 0.15; ey += (Math.random() - 0.5) * mh * 0.15; meteor.active = true; meteor.startX = sx; meteor.startY = sy; meteor.endX = ex; meteor.endY = ey; meteor.progress = 0; meteor.speed = 0.6 + Math.random() * 0.4; const colorRoll = Math.random(); if (colorRoll < 0.5) { meteor.r = 255; meteor.g = 240; meteor.b = 220; } else if (colorRoll < 0.8) { meteor.r = 220; meteor.g = 235; meteor.b = 255; } else { meteor.r = 255; meteor.g = 255; meteor.b = 255; } } if (meteor.active) { const totalDist = Math.sqrt((meteor.endX - meteor.startX) ** 2 + (meteor.endY - meteor.startY) ** 2); meteor.progress += (meteor.speed * 1000 / 60) / totalDist; if (meteor.progress >= 1) { meteor.active = false; nextMeteorTime.current = timestamp + 20000 + Math.random() * 30000; } } const rx = rotationX.current, ry = rotationY.current + galaxyRotation.current; const cosX = Math.cos(rx), sinX = Math.sin(rx), cosY = Math.cos(ry), sinY = Math.sin(ry); const glowTex = glowTexRef.current; const fc = frameCount.current++; ctx.globalCompositeOperation = 'source-over'; const bgUpdateInterval = rp < 1 ? 1 : 90; if (fc % bgUpdateInterval === 0 || !bgStarsOffscreen.current) renderBgStarsToOffscreen(elapsed, rp, w, h, dpr); if (bgStarsOffscreen.current) ctx.drawImage(bgStarsOffscreen.current, 0, 0); ctx.globalCompositeOperation = 'lighter'; const volMinAlpha = 0.004; const cullMargin = Math.max(50, 100 / zoom); for (let i = 0; i < volumeDustRef.current.length; i++) { const p = volumeDustRef.current[i]; if (p.baseAlpha < volMinAlpha) continue; const result = project3D(p.x, p.y, p.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -cullMargin || result.px > w + cullMargin || result.py < -cullMargin || result.py > h + cullMargin) continue; const breathe = 1 + Math.sin(elapsed * 0.5 + p.phase) * 0.06; const alpha = Math.min(1, p.baseAlpha * breathe) * rp; if (alpha < volMinAlpha) continue; const jx = Math.sin(elapsed * p.jxSpeed + p.jxPhase) * JITTER_AMPLITUDE * 0.5, jy = Math.cos(elapsed * p.jySpeed + p.jyPhase) * JITTER_AMPLITUDE * 0.5; ctx.globalAlpha = alpha; ctx.fillStyle = `rgb(${p.r},${p.g},${p.b})`; ctx.beginPath(); ctx.arc(result.px + jx, result.py + jy, p.size * 0.45, 0, Math.PI * 2); ctx.fill(); } ctx.globalCompositeOperation = 'lighter'; for (let i = 0; i < haloRef.current.length; i++) { const p = haloRef.current[i]; const result = project3D(p.x, p.y, p.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -50 || result.px > w + 50 || result.py < -50 || result.py > h + 50) continue; const alpha = Math.min(0.25, p.baseAlpha * (1 + Math.sin(elapsed * 0.6 + p.phase) * 0.08)) * rp; if (alpha < 0.003) continue; const jx = Math.sin(elapsed * p.jxSpeed + p.jxPhase) * JITTER_AMPLITUDE, jy = Math.cos(elapsed * p.jySpeed + p.jyPhase) * JITTER_AMPLITUDE; ctx.globalAlpha = alpha; ctx.fillStyle = `rgb(${p.r},${p.g},${p.b})`; ctx.beginPath(); ctx.arc(result.px + jx, result.py + jy, p.size * 0.6, 0, Math.PI * 2); ctx.fill(); } const minAlpha = 0.022; const spiralProj: { px: number; py: number; depth: number; size: number; alpha: number; r: number; g: number; b: number; idx: number; radius3D: number; phase: number; jxSpeed: number; jySpeed: number; jxPhase: number; jyPhase: number }[] = []; for (let i = 0; i < spiralRef.current.length; i++) { const p = spiralRef.current[i]; if (p.baseAlpha < minAlpha) continue; const result = project3D(p.x, p.y, p.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -cullMargin || result.px > w + cullMargin || result.py < -cullMargin || result.py > h + cullMargin) continue; const breathe = 1 + Math.sin(elapsed * 1.3 + p.phase) * 0.06; const alpha = Math.min(1, p.baseAlpha * breathe) * rp; if (alpha < minAlpha) continue; const jx = Math.sin(elapsed * p.jxSpeed + p.jxPhase) * JITTER_AMPLITUDE, jy = Math.cos(elapsed * p.jySpeed + p.jyPhase) * JITTER_AMPLITUDE; const spx = result.px + jx, spy = result.py + jy; spiralProj.push({ idx: i, px: spx, py: spy, depth: result.depth, size: p.size, alpha, r: p.r, g: p.g, b: p.b, radius3D: p.radius3D, phase: p.phase, jxSpeed: p.jxSpeed, jySpeed: p.jySpeed, jxPhase: p.jxPhase, jyPhase: p.jyPhase }); } spiralProj.sort((a, b) => b.depth - a.depth); for (const sp of spiralProj) { const glowR = sp.size * 4.5; ctx.globalAlpha = sp.alpha; if (glowTex) ctx.drawImage(glowTex, sp.px - glowR, sp.py - glowR, glowR * 2, glowR * 2); if (sp.alpha > 0.10) { ctx.globalAlpha = Math.min(1, sp.alpha * 1.2); ctx.fillStyle = `rgb(${sp.r},${sp.g},${sp.b})`; ctx.beginPath(); ctx.arc(sp.px, sp.py, sp.size * 0.4, 0, Math.PI * 2); ctx.fill(); } } ctx.globalCompositeOperation = 'source-over'; for (let i = 0; i < dustLaneRef.current.length; i++) { const p = dustLaneRef.current[i]; if (p.baseAlpha < minAlpha * 0.4) continue; const result = project3D(p.x, p.y, p.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -cullMargin || result.px > w + cullMargin || result.py < -cullMargin || result.py > h + cullMargin) continue; const breathe = 1 + Math.sin(elapsed * 0.9 + p.phase) * 0.04; const alpha = Math.min(1, p.baseAlpha * breathe) * rp; if (alpha < 0.003) continue; const jx = Math.sin(elapsed * p.jxSpeed + p.jxPhase) * JITTER_AMPLITUDE * 0.5, jy = Math.cos(elapsed * p.jySpeed + p.jyPhase) * JITTER_AMPLITUDE * 0.5; ctx.globalAlpha = alpha; ctx.fillStyle = `rgb(${p.r},${p.g},${p.b})`; ctx.beginPath(); ctx.arc(result.px + jx, result.py + jy, p.size * 0.55, 0, Math.PI * 2); ctx.fill(); } ctx.globalCompositeOperation = 'lighter'; for (let i = 0; i < hiiRef.current.length; i++) { const p = hiiRef.current[i]; if (p.baseAlpha < minAlpha) continue; const result = project3D(p.x, p.y, p.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -cullMargin || result.px > w + cullMargin || result.py < -cullMargin || result.py > h + cullMargin) continue; const breathe = 1 + Math.sin(elapsed * 1.5 + p.phase) * 0.08; const alpha = Math.min(1, p.baseAlpha * breathe) * rp; if (alpha < minAlpha) continue; const jx = Math.sin(elapsed * p.jxSpeed + p.jxPhase) * JITTER_AMPLITUDE, jy = Math.cos(elapsed * p.jySpeed + p.jyPhase) * JITTER_AMPLITUDE; const spx = result.px + jx, spy = result.py + jy; const projectedSize = p.size / Math.max(result.depth + 2.2 / zoom, 0.3) * projScale * 0.3; if (alpha > 0.22 && glowTex && projectedSize > 0.5) { const glowR = p.size * 3.5; ctx.globalAlpha = alpha * 0.6; ctx.drawImage(glowTex, spx - glowR, spy - glowR, glowR * 2, glowR * 2); } ctx.globalAlpha = Math.min(1, alpha * 1.1); ctx.fillStyle = `rgb(${p.r},${p.g},${p.b})`; ctx.beginPath(); ctx.arc(spx, spy, p.size * 0.35, 0, Math.PI * 2); ctx.fill(); } for (let i = 0; i < interArmRef.current.length; i++) { const p = interArmRef.current[i]; if (p.baseAlpha < minAlpha) continue; const result = project3D(p.x, p.y, p.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -cullMargin || result.px > w + cullMargin || result.py < -cullMargin || result.py > h + cullMargin) continue; const breathe = 1 + Math.sin(elapsed * 1.1 + p.phase) * 0.05; const alpha = Math.min(1, p.baseAlpha * breathe) * rp; if (alpha < minAlpha) continue; const jx = Math.sin(elapsed * p.jxSpeed + p.jxPhase) * JITTER_AMPLITUDE * 0.7, jy = Math.cos(elapsed * p.jySpeed + p.jyPhase) * JITTER_AMPLITUDE * 0.7; ctx.globalAlpha = alpha; ctx.fillStyle = `rgb(${p.r},${p.g},${p.b})`; ctx.beginPath(); ctx.arc(result.px + jx, result.py + jy, p.size * 0.5, 0, Math.PI * 2); ctx.fill(); } { const spikeCandidates: { px: number; py: number; brightness: number }[] = []; for (let i = 0; i < hiiRef.current.length; i++) { const p = hiiRef.current[i]; if (p.baseAlpha < 0.25) continue; const result = project3D(p.x, p.y, p.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < 0 || result.px > w || result.py < 0 || result.py > h) continue; const jx = Math.sin(elapsed * p.jxSpeed + p.jxPhase) * JITTER_AMPLITUDE, jy = Math.cos(elapsed * p.jySpeed + p.jyPhase) * JITTER_AMPLITUDE; const projectedSize = p.size / Math.max(result.depth + 2.2 / zoom, 0.3) * projScale * 0.3; spikeCandidates.push({ px: result.px + jx, py: result.py + jy, brightness: p.baseAlpha * projectedSize }); } spikeCandidates.sort((a, b) => b.brightness - a.brightness); const topSpikes = spikeCandidates.slice(0, 12); ctx.globalCompositeOperation = 'lighter'; for (const s of topSpikes) { const spikeLen = 2.5 + s.brightness * 8; const alpha = Math.min(0.55, 0.08 + s.brightness * 1.2); ctx.globalAlpha = alpha; const axes: [number, number][] = [[1, 0], [0, 1]]; for (const [ax, ay] of axes) { const spikeGrad = ctx.createLinearGradient(s.px - ax * spikeLen, s.py - ay * spikeLen, s.px + ax * spikeLen, s.py + ay * spikeLen); spikeGrad.addColorStop(0, 'rgba(255,255,255,0)'); spikeGrad.addColorStop(0.45, `rgba(255,255,255,${alpha.toFixed(3)})`); spikeGrad.addColorStop(0.5, 'rgba(255,255,255,0.95)'); spikeGrad.addColorStop(0.55, `rgba(255,255,255,${alpha.toFixed(3)})`); spikeGrad.addColorStop(1, 'rgba(255,255,255,0)'); ctx.strokeStyle = spikeGrad; ctx.lineWidth = 0.4; ctx.beginPath(); ctx.moveTo(s.px - ax * spikeLen, s.py - ay * spikeLen); ctx.lineTo(s.px + ax * spikeLen, s.py + ay * spikeLen); ctx.stroke(); ctx.lineWidth = 1.5; ctx.strokeStyle = `rgba(255,255,255,${(alpha * 0.2).toFixed(3)})`; ctx.beginPath(); ctx.moveTo(s.px - ax * spikeLen * 0.5, s.py - ay * spikeLen * 0.5); ctx.lineTo(s.px + ax * spikeLen * 0.5, s.py + ay * spikeLen * 0.5); ctx.stroke(); } } } for (let i = 0; i < planetsRef.current.length; i++) { const planet = planetsRef.current[i]; for (let t = 0; t < PLANET_TRAIL_LENGTH; t++) { const idx = (planet.trailHead + t) % PLANET_TRAIL_LENGTH; const tp = planet.trail[idx]; const result = project3D(tp.x, tp.y, tp.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -20 || result.px > w + 20 || result.py < -20 || result.py > h + 20) continue; const trailAlpha = (t / (PLANET_TRAIL_LENGTH - 1)) * 0.22 * rp; if (trailAlpha < 0.003) continue; const trailSize = planet.size * 0.35 * (0.3 + 0.7 * (t / PLANET_TRAIL_LENGTH)); ctx.globalAlpha = trailAlpha; ctx.fillStyle = `rgb(${planet.r},${planet.g},${planet.b})`; ctx.beginPath(); ctx.arc(result.px, result.py, trailSize, 0, Math.PI * 2); ctx.fill(); } } ctx.globalCompositeOperation = 'source-over'; flyOutScreen.current = null; const nodeProj: { n: PhotoNode; px: number; py: number; depth: number; size: number; alpha: number }[] = []; const MAX_LINES = 0; for (let i = 0; i < photoNodesRef.current.length; i++) { const n = photoNodesRef.current[i]; const result = project3D(n.x, n.y, n.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; if (result.px < -30 || result.px > w + 30 || result.py < -30 || result.py > h + 30) continue; const breathe = 1 + Math.sin(elapsed * 2.0 + n.phase) * 0.10; const targetScale = hoveredRef.current === n.id ? 1.6 : 1.0; n.hoverScale += (targetScale - n.hoverScale) * 0.18; if (n.clickFlash > 0.001) { n.clickFlash *= 0.82; if (n.clickFlash < 0.001) n.clickFlash = 0; } const highlightedIdx = useAppStore.getState().highlightedIndex; const targetFly = (highlightedIdx !== null && highlightedIdx === n.fileIndex) ? 1.0 : 0.0; n.flyProgress += (targetFly - n.flyProgress) * 0.08; const jx = Math.sin(elapsed * n.jxSpeed + n.jxPhase) * JITTER_AMPLITUDE, jy = Math.cos(elapsed * n.jySpeed + n.jyPhase) * JITTER_AMPLITUDE; const depthScale = 1 / Math.max(result.depth + 2.2 / zoom, 0.3); let px = result.px + jx, py = result.py + jy, size = n.baseSize * n.hoverScale * depthScale * 0.62, depth = result.depth; if (n.flyProgress > 0.001) { const fp = n.flyProgress, targetX = cx, targetY = 52; const t = 1 - Math.pow(1 - fp, 3); px = px + (targetX - px) * t; py = py + (targetY - py) * t; size = size * (1 + fp * 3.5); depth = 2.5; if (fp > 0.3) flyOutScreen.current = { px, py, fileIndex: n.fileIndex, size }; } const alphaBright = hoveredRef.current === n.id ? 1.8 : 1.0; const flash = 1 + n.clickFlash * 3.5; const alpha = Math.min(1, 0.90 * breathe * alphaBright * flash) * rp; if (alpha < 0.01) continue; nodeProj.push({ n, px, py, depth, size: Math.max(0.6, size), alpha }); } nodeProj.sort((a, b) => a.depth - b.depth); ctx.globalCompositeOperation = 'lighter'; const lineLimit = Math.min(MAX_LINES, nodeProj.length); for (let i = 0; i < lineLimit; i++) { const { n, px, py, depth } = nodeProj[i]; const lineAlpha = (0.04 + n.radius3D * 0.02 + (1 / Math.max(depth + 2.2 / zoom, 0.3)) * 0.015) * rp; ctx.globalAlpha = Math.min(0.07, lineAlpha); ctx.strokeStyle = `rgb(${n.r},${n.g},${n.b})`; ctx.lineWidth = 0.15; ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(px, py); ctx.stroke(); } ctx.globalCompositeOperation = 'source-over'; { const videoNodes = nodeProj.filter(p => p.n.isVideo); if (videoNodes.length > 1) { const CONNECT_DIST = Math.min(w, h) * 0.18 * VIDEO_CONNECT_DIST_FACTOR; ctx.globalCompositeOperation = 'lighter'; for (let a = 0; a < videoNodes.length; a++) { for (let b = a + 1; b < videoNodes.length; b++) { const dx = videoNodes[a].px - videoNodes[b].px, dy = videoNodes[a].py - videoNodes[b].py, dist = Math.sqrt(dx * dx + dy * dy); if (dist < CONNECT_DIST) { const la = (1 - dist / CONNECT_DIST) * 0.15 * rp; if (la > 0.005) { ctx.globalAlpha = la; ctx.strokeStyle = 'rgba(120,200,255,0.8)'; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(videoNodes[a].px, videoNodes[a].py); ctx.lineTo(videoNodes[b].px, videoNodes[b].py); ctx.stroke(); } } } } ctx.globalCompositeOperation = 'source-over'; } } for (let i = 0; i < nodeProj.length; i++) { const { n, px, py, size, alpha } = nodeProj[i]; const glowR = size * 5.0; if (n.isVideo && size > 0.8) { const vgR = size * 8; ctx.globalAlpha = alpha * 0.25; if (glowTex) ctx.drawImage(glowTex, px - vgR, py - vgR, vgR * 2, vgR * 2); ctx.globalAlpha = alpha * 0.35; ctx.fillStyle = 'rgba(160,225,255,0.5)'; ctx.beginPath(); ctx.moveTo(px, py - size * 1.1); ctx.lineTo(px + size * 0.45, py); ctx.lineTo(px, py + size * 1.1); ctx.lineTo(px - size * 0.45, py); ctx.closePath(); ctx.fill(); } if (size > 0.8) { ctx.globalAlpha = alpha * 0.8; if (glowTex) ctx.drawImage(glowTex, px - glowR, py - glowR, glowR * 2, glowR * 2); } ctx.globalAlpha = Math.min(1, alpha * 1.1); ctx.fillStyle = `rgb(${n.r},${n.g},${n.b})`; ctx.beginPath(); ctx.arc(px, py, size * 0.75, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = Math.min(1, alpha * 1.1); ctx.fillStyle = '#ffffff'; ctx.beginPath(); ctx.arc(px, py, size * 0.22, 0, Math.PI * 2); ctx.fill(); if (n.flyProgress > 0.05) { const fp = n.flyProgress; ctx.globalCompositeOperation = 'lighter'; const pulse = 0.7 + 0.3 * Math.sin(elapsed * 2.5 + n.phase); const lineAlpha = Math.min(1, fp * 1.0) * pulse; ctx.globalAlpha = lineAlpha; ctx.strokeStyle = `rgba(255,210,122,${(0.9 * fp).toFixed(2)})`; ctx.lineWidth = 1.8 + fp * 2.5; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(px, py); ctx.stroke(); ctx.lineCap = 'butt'; ctx.globalAlpha = lineAlpha * 0.4; ctx.strokeStyle = `rgba(255,240,200,${(0.3 * fp).toFixed(2)})`; ctx.lineWidth = 5 + fp * 8; ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(px, py); ctx.stroke(); ctx.globalAlpha = lineAlpha * 0.7; ctx.fillStyle = '#fffef0'; ctx.beginPath(); ctx.arc(px, py, 2.5 + fp * 4, 0, Math.PI * 2); ctx.fill(); ctx.globalCompositeOperation = 'source-over'; } } ctx.globalCompositeOperation = 'lighter'; for (let i = 0; i < planetsRef.current.length; i++) { const planet = planetsRef.current[i]; const result = project3D(planet.x, planet.y, planet.z, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (!result) continue; const depthScale = 1 / Math.max(result.depth + 2.2 / zoom, 0.3); const projectedSize = planet.size * depthScale * 0.5; if (glowTex && projectedSize > 0.4) { const glowR = projectedSize * 4.0; ctx.globalAlpha = 0.35 * rp; ctx.drawImage(glowTex, result.px - glowR, result.py - glowR, glowR * 2, glowR * 2); } ctx.globalAlpha = Math.min(1, 0.7 * rp); ctx.fillStyle = `rgb(${planet.r},${planet.g},${planet.b})`; ctx.beginPath(); ctx.arc(result.px, result.py, Math.max(0.5, projectedSize), 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = Math.min(1, 0.55 * rp); ctx.fillStyle = '#ffffff'; ctx.beginPath(); ctx.arc(result.px, result.py, Math.max(0.15, projectedSize * 0.25), 0, Math.PI * 2); ctx.fill(); } for (let i = 0; i < bokehRef.current.length; i++) { const b = bokehRef.current[i]; const px = b.x * w, py = b.y * h; const breathe = 0.7 + 0.3 * Math.sin(elapsed * b.twinkleSpeed + b.twinklePhase); const alpha = b.baseAlpha * breathe * rp; if (alpha < 0.003) continue; const blurLayers = [{ r: b.size * 1.0, a: alpha }, { r: b.size * 0.75, a: alpha * 0.7 }, { r: b.size * 0.5, a: alpha * 0.5 }, { r: b.size * 0.3, a: alpha * 0.35 }, { r: b.size * 0.12, a: alpha * 0.2 }]; for (const layer of blurLayers) { ctx.globalAlpha = Math.min(1, layer.a); ctx.fillStyle = `rgb(${b.r},${b.g},${b.b})`; ctx.beginPath(); ctx.arc(px, py, layer.r, 0, Math.PI * 2); ctx.fill(); } } if (meteorRef.current.active) { const m = meteorRef.current; const mx = m.startX + (m.endX - m.startX) * m.progress, my = m.startY + (m.endY - m.startY) * m.progress; const dx = m.endX - m.startX, dy = m.endY - m.startY; const dist = Math.sqrt(dx * dx + dy * dy); const nx = dx / dist, ny = dy / dist; const tailLen = Math.min(w, h) * 0.22; const streakGrad = ctx.createLinearGradient(mx, my, mx - nx * tailLen, my - ny * tailLen); streakGrad.addColorStop(0, `rgba(${m.r},${m.g},${m.b},0.95)`); streakGrad.addColorStop(0.05, `rgba(${m.r},${m.g},${m.b},0.7)`); streakGrad.addColorStop(0.2, `rgba(${m.r},${m.g},${m.b},0.3)`); streakGrad.addColorStop(0.5, `rgba(${m.r},${m.g},${m.b},0.06)`); streakGrad.addColorStop(1, 'rgba(0,0,0,0)'); ctx.strokeStyle = streakGrad; ctx.lineWidth = 2.5; ctx.beginPath(); ctx.moveTo(mx, my); ctx.lineTo(mx - nx * tailLen, my - ny * tailLen); ctx.stroke(); const headGlow = ctx.createRadialGradient(mx, my, 0, mx, my, 12); headGlow.addColorStop(0, `rgba(${m.r},${m.g},${m.b},0.9)`); headGlow.addColorStop(0.3, `rgba(${m.r},${m.g},${m.b},0.4)`); headGlow.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = headGlow; ctx.beginPath(); ctx.arc(mx, my, 12, 0, Math.PI * 2); ctx.fill(); const particleCount = 18; for (let p = 0; p < particleCount; p++) { const t = p / particleCount; const px = mx - nx * tailLen * t, py = my - ny * tailLen * t; const pAlpha = (1 - t) * 0.3 * (1 - m.progress * 0.5); if (pAlpha < 0.01) continue; const pSize = 0.8 + (1 - t) * 2.5; ctx.globalAlpha = pAlpha; ctx.fillStyle = `rgb(${m.r},${m.g},${m.b})`; ctx.beginPath(); ctx.arc(px + (Math.random() - 0.5) * 8, py + (Math.random() - 0.5) * 8, pSize, 0, Math.PI * 2); ctx.fill(); } } if (rp < 0.02) { ctx.globalCompositeOperation = 'lighter'; const hintR = Math.min(w, h) * 0.08; const pulse = 0.4 + 0.6 * Math.sin(elapsed * 2.5); const grad = ctx.createRadialGradient(cx, cy, hintR * 0.6, cx, cy, hintR); grad.addColorStop(0, 'rgba(255,240,200,0)'); grad.addColorStop(0.5, `rgba(255,240,200,${(0.08 * pulse).toFixed(3)})`); grad.addColorStop(1, 'rgba(255,240,200,0)'); ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(cx, cy, hintR, 0, Math.PI * 2); ctx.fill(); } ctx.globalCompositeOperation = 'lighter'; const coreR = Math.min(w, h) * 0.13 * CORE_SCALE; const coreLayers = [{ r: coreR * 2.2, stops: [[0, 'rgba(255,235,190,0.025)'], [0.15, 'rgba(255,220,160,0.012)'], [0.40, 'rgba(180,150,100,0.004)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: coreR * 1.2, stops: [[0, 'rgba(255,240,210,0.08)'], [0.12, 'rgba(255,225,180,0.04)'], [0.40, 'rgba(200,165,120,0.008)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: coreR * 0.55, stops: [[0, 'rgba(255,248,235,0.25)'], [0.10, 'rgba(255,240,215,0.14)'], [0.35, 'rgba(255,220,170,0.04)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: coreR * 0.20, stops: [[0, 'rgba(255,252,248,0.65)'], [0.10, 'rgba(255,248,235,0.35)'], [0.35, 'rgba(255,235,195,0.08)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: coreR * 0.06, stops: [[0, 'rgba(255,255,255,1.0)'], [0.12, 'rgba(255,254,250,0.80)'], [0.40, 'rgba(255,245,215,0.15)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }]; for (let k = 0; k < coreLayers.length; k++) { const layer = coreLayers[k]; const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, layer.r); for (let s = 0; s < layer.stops.length; s++) grad.addColorStop(layer.stops[s][0] as number, layer.stops[s][1] as string); ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(cx, cy, layer.r, 0, Math.PI * 2); ctx.fill(); } const flareBase = Math.min(w, h) * 0.20; const spikeAngles6 = [0, Math.PI / 3, 2 * Math.PI / 3, Math.PI, 4 * Math.PI / 3, 5 * Math.PI / 3]; for (let s = 0; s < 6; s++) { const angle = spikeAngles6[s]; const cosA = Math.cos(angle), sinA = Math.sin(angle); const spikeLen = flareBase * (s % 2 === 0 ? 1.0 : 0.65); const spikeGrad = ctx.createLinearGradient(cx, cy, cx + cosA * spikeLen, cy + sinA * spikeLen); spikeGrad.addColorStop(0, 'rgba(255,255,255,0.8)'); spikeGrad.addColorStop(0.03, 'rgba(255,250,240,0.4)'); spikeGrad.addColorStop(0.12, 'rgba(255,220,180,0.10)'); spikeGrad.addColorStop(0.35, 'rgba(180,150,200,0.02)'); spikeGrad.addColorStop(0.7, 'rgba(0,0,0,0)'); ctx.strokeStyle = spikeGrad; ctx.lineWidth = flareBase * 0.018; ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + cosA * spikeLen, cy + sinA * spikeLen); ctx.stroke(); ctx.lineWidth = flareBase * 0.06; ctx.strokeStyle = spikeGrad; ctx.globalAlpha = 0.18; ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + cosA * spikeLen * 0.5, cy + sinA * spikeLen * 0.5); ctx.stroke(); ctx.globalAlpha = 1; } const reflColors = [{ dist: 1.5, r: 0.08, color: 'rgba(255,200,150,0.15)' }, { dist: 2.2, r: 0.05, color: 'rgba(150,200,255,0.10)' }, { dist: 3.0, r: 0.1, color: 'rgba(200,255,220,0.07)' }, { dist: 4.0, r: 0.04, color: 'rgba(255,180,200,0.08)' }]; for (const refl of reflColors) { const rr = flareBase * refl.r; for (const sign of [-1, 1]) { const rx = cx + sign * flareBase * refl.dist, ry = cy; const rgrad = ctx.createRadialGradient(rx, ry, 0, rx, ry, rr); rgrad.addColorStop(0, refl.color); rgrad.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = rgrad; ctx.beginPath(); ctx.arc(rx, ry, rr, 0, Math.PI * 2); ctx.fill(); } } const whiteCoreR = flareBase * 0.04; const whiteCore = ctx.createRadialGradient(cx, cy, 0, cx, cy, whiteCoreR); whiteCore.addColorStop(0, 'rgba(255,255,255,1)'); whiteCore.addColorStop(0.15, 'rgba(255,255,255,0.9)'); whiteCore.addColorStop(0.5, 'rgba(255,240,220,0.3)'); whiteCore.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = whiteCore; ctx.beginPath(); ctx.arc(cx, cy, whiteCoreR, 0, Math.PI * 2); ctx.fill(); { const vgCenter = project3D(0, VIDEO_GALAXY_Y_OFFSET, VIDEO_GALAXY_Z_OFFSET, cx, cy, projScale, cosX, sinX, cosY, sinY, zoom); if (vgCenter) { const vgCoreR = Math.min(w, h) * 0.13 * CORE_SCALE * VIDEO_CORE_SCALE; const vgLayers = [{ r: vgCoreR * 1.8, stops: [[0, 'rgba(160,210,240,0.018)'], [0.15, 'rgba(140,190,230,0.010)'], [0.40, 'rgba(100,140,180,0.003)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: vgCoreR * 0.9, stops: [[0, 'rgba(180,220,245,0.05)'], [0.12, 'rgba(160,200,235,0.03)'], [0.40, 'rgba(120,160,200,0.006)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: vgCoreR * 0.35, stops: [[0, 'rgba(190,230,250,0.18)'], [0.10, 'rgba(170,210,240,0.10)'], [0.35, 'rgba(140,185,220,0.03)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: vgCoreR * 0.12, stops: [[0, 'rgba(200,240,255,0.45)'], [0.10, 'rgba(180,225,250,0.25)'], [0.35, 'rgba(150,200,235,0.06)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }, { r: vgCoreR * 0.04, stops: [[0, 'rgba(220,245,255,0.85)'], [0.12, 'rgba(200,235,255,0.60)'], [0.40, 'rgba(160,215,245,0.10)'], [1, 'rgba(0,0,0,0)']] as [number, string][] }]; for (let k = 0; k < vgLayers.length; k++) { const layer = vgLayers[k]; const grad = ctx.createRadialGradient(vgCenter.px, vgCenter.py, 0, vgCenter.px, vgCenter.py, layer.r); for (let s = 0; s < layer.stops.length; s++) grad.addColorStop(layer.stops[s][0] as number, layer.stops[s][1] as string); ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(vgCenter.px, vgCenter.py, layer.r, 0, Math.PI * 2); ctx.fill(); } } } ctx.globalCompositeOperation = 'source-over'; ctx.globalAlpha = 1; animRef.current = requestAnimationFrame(render); }; animRef.current = requestAnimationFrame(render); return () => cancelAnimationFrame(animRef.current); }, [manifest]); return (<canvas ref={canvasRef} onMouseMove={handleMouseMove} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseLeave={handleMouseLeave} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} style={{ position: 'absolute', inset: 0, zIndex: 0, cursor: 'default' }} />); }